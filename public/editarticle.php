...
if (!isset($_GET['id'])) {
	...
	if (!$session->isAdmin()) {
		...
		// fetch article data
		$sql = "SELECT id, title, summary, DATE_FORMAT(date_posted, \"%d-%m-%Y\")" .
		...
		if ($numrows != 0) {
    		while ($row = mysql_fetch_assoc($result)) {
        		...
        		$post_date  = $row['newdate'];
        		$post_text  = htmlspecialchars_decode($row['content']);
...
<table>
	...
	<tr>
		...
		<td><input class="formInputText" type="text" name="articletitle"
			maxlength="80" value="<?php echo $post_title?>"></td>
   	</tr>
   	<tr>
		<td><label class="formLabelText" for="articlesummary">Summary:</label></td>
		<td>
			<textarea class="formTextArea" name="articlesummary" rows="5" cols="40"><?php echo $post_summary?></textarea>
		</td>
	</tr>
	<tr>
		<td><label class="formLabelText" for="articlecategory">Category:</label></td>	
		<td>
			<select class="select" multiple name="articlecategory[]" size="6">
				<?php echo $post_categories_html?>
			</select>
		</td>
	</tr> 
	<tr>
		...
		<td>		
			<input class="formInputText" id="articledate" name="articledate"
				type="text" value="<?php echo $post_date?>">
